      ...
      <div :class="addressContainer">
        ...
        <template v-if="!onlyProvince">
          ...
          <ul v-if="tab == 3 && !hideArea">
            <li v-for="(item, index) in areas" :class="{'active': item == currentArea}" @click="chooseArea(item,index)">{{ item }}</li>
          </ul>
        </template>
      </div>
    </template>
...
<script>
import DISTRICTS from '../assets/js/occType';
...
export default {
  ...
  methods: {
    ...
    chooseArea(name,index) {
      this.currentArea = name
      this.code = index
      bus.$emit('selectedOcc')
      bus.$emit('currentOcc',this.currentArea)
      bus.$emit('currentOccCode',this.code)
    },
